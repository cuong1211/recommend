<script>
    ...
    $('.btn-import').on('click', function() {
        $('#kt_modal_import_customer').modal('show');
    });
    $('.btn-import-rate').on('click', function() {
        $('#kt_modal_import_rate').modal('show');
    });
    ...
    $(document).on('click', '.btn-add', function (e) {
        ...
        form_reset();
        let modal = $('#kt_modal_add_customer_form');
        modal.find('.modal-title').text('Thêm ngươi dùng');
    ...
    $('#kt_modal_import_customer_form').on('submit', function (e){
        e.preventDefault();
        let formdata = new FormData(),
            type = 'POST',
            url = "{{route('user.import')}}" ;
        formdata.append('file', $('#kt_modal_import_customer_form input[name=file]')[0].files[0]);
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if (data.type == 'success') {
                    dt.ajax.reload(null, false);
                    $('#kt_modal_import_customer_form').trigger('reset');
                    $('#kt_modal_import_customer').modal('hide');
                }
            },
            error: function(data) {
                let errors = data.responseJSON.errors;
    ...
    $('#kt_modal_import_rate_form').on('submit', function (e){
        e.preventDefault();
        let formdata = new FormData(),
            type = 'POST',
            url = "{{route('rate.import')}}" ;
        formdata.append('file', $('#kt_modal_import_rate_form input[name=file]')[0].files[0]);
        console.log(formdata);
        $.ajax({
            url: url,
            headers: {
            ...
            success: function(data){
                ...
                if (data.type == 'success') {
                    dt.ajax.reload(null, false);
                    $('#kt_modal_import_rate_form').trigger('reset');
                    $('#kt_modal_import_rate').modal('hide');
                }
            },
            error: function(data) {
                let errors = data.responseJSON.errors;
